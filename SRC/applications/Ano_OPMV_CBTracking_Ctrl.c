...
//
#define CBT_KP                  (0.40f)  //比例项
#define CBT_KD                  (0.10f)  //微分项
#define CBT_KF                  (0.10f)  //前馈项
...
void ANO_CBTracking_Ctrl(u8 *dT_ms,u8 en)
{
	...
	if(en)
	{
		if(opmv.cb.sta == 0){
			k=1;
			ano_opmv_cbt_ctrl.exp_velocity_h_cmps[0] = 25;}
		else if(opmv.cb.sta == 3)
			{
			one_key_land();//状态3，在p正上方，一件降落
			}
		else{
			//距离偏差PD控制和速度前馈
	...
	{
		ano_opmv_cbt_ctrl.exp_velocity_h_cmps[0] = ano_opmv_cbt_ctrl.exp_velocity_h_cmps[1] = 0;//复位
