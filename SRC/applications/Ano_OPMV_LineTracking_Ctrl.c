...
#define LT_KP                  (0.20f)  //比例项
#define LT_KD                  (0.02f)  //微分项
u8 p=0;
...
static void ANO_LTracking_Decoupling(u8 *dT_ms,float rol_degs,float pit_degs)
{
	...
	{
		//延迟一定时间
		ano_opmv_lt_ctrl.exp_velocity_h_cmps[0] = 20;//往前飞
...
void ANO_LT_StepProcedure(u8 *dT_ms)
{
	...
	switch(step_pro_sta)
	{
		...
		{
			//刹车减速
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[0] =6;//识别到直角后减速前进的速度cm/s
	...
	if(en)
		{
			...
			+ LT_KD *ano_opmv_lt_ctrl.ground_pos_err_d_h_cmps;//x方向速度赋值，pd控制
		

			if(opmv.lt.sta == 1)
			{
				//
				ano_opmv_lt_ctrl.exp_yaw_pal_dps = -opmv.lt.angle *6;//偏航角
				ano_opmv_lt_ctrl.exp_yaw_pal_dps = LIMIT(ano_opmv_lt_ctrl.exp_yaw_pal_dps,-90,90);
			}
			else
			{
				ano_opmv_lt_ctrl.exp_yaw_pal_dps = 0;
